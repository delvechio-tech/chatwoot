class Api::V1::Accounts::InboxesController < Api::V1::Accounts::BaseController
  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      @inbox.save!
      ensure_current_user_as_quepasa_agent(channel)
    end
  end
  ...
  def quepasa_qr
    return render json: { error: 'Quepasa settings are only available for WhatsApp API inboxes' }, status: :bad_request unless quepasa_channel?
  ...
  def quepasa_channel?
    @inbox.whatsapp? && @inbox.channel.provider == 'quepasa'
  end

  def ensure_current_user_as_quepasa_agent(channel)
    return unless channel.is_a?(Channel::Whatsapp) && channel.provider == 'quepasa'
    return if Current.user.blank?

    @inbox.inbox_members.find_or_create_by!(user: Current.user)
  end
end
